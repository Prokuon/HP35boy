...
void lcd_init(bool powered = true)
{
    ...
    // Power switch
    byte x = powered ? 58 : 62;
    arduboy.drawLine(116, x, 116, x + 4);

    if (powered)
    {
        // Draw selection
        byte ix = 128 - keyboard[currentKey].x - keyboard[currentKey].size + 2;
        byte iy = 64 - keyboard[currentKey].y - keyboard[currentKey].size + 1;
        byte iw = 11 - 2;
        byte ih = keyboard[currentKey].size - 2;
...
void print_lcd(unsigned char m) 
{
    ...
    if (!space)
        sprites.drawSelfMasked(118, (chr_prtd * 4) + 2, font, m - '0');
    chr_prtd++;
}
